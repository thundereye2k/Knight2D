...
{
	static string version = "2.1.3";// ALWAYS UPDATE
	...
	void OnEnable ()
	{
		...
		futureUpdates.targetMethod = PlannedFeatures;
	...
	void BeginScrollView ( ref Vector2 scrollPos )
	{
	...
	void HowTo ()
	{
		BeginScrollView( ref howTo.scrollPosition );
	...
	void OverviewPage ()
	{
		BeginScrollView( ref overview.scrollPosition );
	...
	{
		BeginScrollView( ref overview_USB.scrollPosition );
	...
	{
		BeginScrollView( ref overview_DSF.scrollPosition );
	...
	{
		BeginScrollView( ref overview_ASH.scrollPosition );
	...
	{
		BeginScrollView( ref overview_SFF.scrollPosition );
	...
	{
		BeginScrollView( ref documentation_US.scrollPosition );
	...
	{
		BeginScrollView( ref documentation_USB.scrollPosition );
		...
		EditorGUILayout.LabelField( "Ultimate Status", itemHeaderStyle );
		EditorGUILayout.LabelField( "Each status that is displayed on the Ultimate Status Bar inspector is a implementation of the Ultimate Status class. Click <b>More Info</b> below for information on the public functions that available for use through the Ultimate Status class.", paragraphStyle );
	...
	{
		BeginScrollView( ref documentation_ASH.scrollPosition );
	...
	void Extras ()
	{
		BeginScrollView( ref extras.scrollPosition );
		
		EditorGUILayout.LabelField( "Tutorials", sectionHeaderStyle );
		EditorGUILayout.LabelField( "   If you are interested in learning more about how to use the Ultimate Status Bar and all it's components, please visit the link below to visit our YouTube channel where we have a collection of videos that we have made in connection with the Ultimate Status Bar. These videos are designed to help get the Ultimate Status Bar implemented into your project as fast as possible, and give you a good understanding of what you can achieve using it in your projects.", paragraphStyle );

		GUILayout.Space( itemHeaderSpace );

		EditorGUILayout.BeginHorizontal();
		GUILayout.FlexibleSpace();
		if( GUILayout.Button( "YouTube", buttonSize ) )
		...
		EditorGUILayout.LabelField( "Downloads", sectionHeaderStyle );
		EditorGUILayout.LabelField( "   If you are interested in downloading additional textures, prefabs, or scripts that can be used with the Ultimate Status Bar, please visit the link below.", paragraphStyle );

		GUILayout.Space( itemHeaderSpace );

		EditorGUILayout.BeginHorizontal();
		GUILayout.FlexibleSpace();
		if( GUILayout.Button( "Downloads", buttonSize ) )
			Application.OpenURL( "http://www.tankandhealerstudio.com/downloads-for-ultimate-status-bar.html" );
		GUILayout.FlexibleSpace();
		EditorGUILayout.EndHorizontal();
	...
	void OtherProducts ()
	{
		BeginScrollView( ref otherProducts.scrollPosition );
	...
	void Feedback ()
	{
		BeginScrollView( ref feedback.scrollPosition );
	...
	void ChangeLog ()
	{
		BeginScrollView( ref changeLog.scrollPosition );
		...
		EditorGUILayout.LabelField( "  • Updates to the Documentation Window.", paragraphStyle );
		EditorGUILayout.LabelField( "  • Added 13 new textures in the Minimalist style.", paragraphStyle );

		GUILayout.Space( itemHeaderSpace );

		EditorGUILayout.LabelField( "Version 2.1.2", itemHeaderStyle );
		EditorGUILayout.LabelField( "  • Minor updates to prefabs.", paragraphStyle );
		EditorGUILayout.LabelField( "  • Minor change to Documentation Window.", paragraphStyle );
	...
	void ThankYou ()
	{
		BeginScrollView( ref thankYou.scrollPosition );
		
		EditorGUILayout.LabelField( "   We here at Tank & Healer Studio would like to thank you for purchasing the Ultimate Status Bar asset package from the Unity Asset Store. Please click the link below to view a short introductory video for the Ultimate Status Bar.", paragraphStyle );
		...
		if( introVideo != null )
		{
			...
			if( GUILayout.Button( introVideo, GUILayout.Width( 250 ), GUILayout.Height( 140 ) ) )
				Application.OpenURL( "https://www.youtube.com/watch?v=jGzoZ8JQSzk" );

			GUI.backgroundColor = origColor;
		}
		else
		{
			if( GUILayout.Button( "Intro Video", buttonSize ) )
				Application.OpenURL( "https://www.youtube.com/watch?v=jGzoZ8JQSzk" );
		}
		...
		EditorGUILayout.LabelField( "If you have any questions about this product please don't hesitate to contact us at: ", paragraphStyle );
	...
	void VersionChanges ()
	{
		BeginScrollView( ref versionChanges.scrollPosition );
	...
	class UltimateStatusBarInitialLoad
	{
		static UltimateStatusBarInitialLoad ()
		{
			...
			if( !EditorPrefs.HasKey( "UltimateStatusBarVersion" ) )
			{
				...
				img = new WWW( "http://www.tankandhealerstudio.com/uploads/7/7/4/9/77490188/faux-youtube-thumb_orig.jpg" );
